...
function Households(){
    ...
    const [searchHouseholds, setsearchHouseholds] = useState([]);
    ...
    const fetchHouseholdData = async () => {
        ...
        setsearchHouseholds(response.data);
        } catch (error) {
    ...
    const [searchBy, setSearchBy] = useState("lastname"); // Default search by lastname
    const [typeOfInput, setTypeOfInput] = useState("text"); 
    
    // Function to check if an age is within the searched range
    const isAgeInRange = (searchQuery, age) => {
        searchQuery = searchQuery.trim();

        // Match patterns like "1-18", "60-", "-18"
        const rangePattern = /^(\d*)-(\d*)$/;
        const match = searchQuery.match(rangePattern);
        ...
        if (match) {
            ...
            return age === parseInt(searchQuery, 10);
        }
    ...
    const handleSearch = () => {
        const filtered = searchHouseholds.filter(household => {
            const headMember = household.members.find(member => member.isHead === "true");
            const resident = headMember ? residents.find(res => res.id === headMember.residentID) : null;
    
            if (!resident) return false;
    
            if (searchBy === "lastname") {
                return resident.lastname.toLowerCase().includes(searchQuery.toLowerCase());
            } else if (searchBy === "age") {
                const residentAge = calculateAge(resident.birthdate);
                return isAgeInRange(searchQuery, residentAge);
            } else if (searchBy === "monthlyIncome") {
                return isAgeInRange(searchQuery, resident.monthlyIncome);
            } else if (searchBy === "householdSize") {
                return isAgeInRange(searchQuery, household.members.length);
            }
           ...
           <div className="socioeconomiccard">
                ...
                <div>
                    <Card className="h-100">
                        ...
                        <p className="text-xl" id="socioeconomicviewmore" onClick={() => {setFilteredHouseholds(employedHouseholdHead); setsearchHouseholds(employedHouseholdHead); setViewHouseholds("Employed")}}>View More</p>
                    </Card>
                </div>
                <div>
                    <Card className="h-100">
                        ...
                        <p className="text-xl" id="socioeconomicviewmore" onClick={() => {setFilteredHouseholds(unemployedHouseholdHead); setsearchHouseholds(unemployedHouseholdHead);setViewHouseholds("Unemployed")}}>View More</p>
                    </Card>
                </div>
            </div>
                ...
                <div className="householdstopright">
                    ...
                    <div className="dropdown">
                        ...
                        <ul className="dropdown-menu">
                            <li><a className="dropdown-item" onClick={() => {setFilteredHouseholds(fetchedHouseholdData); setViewHouseholds("All"); setSearchBy("lastname"); setTypeOfInput("text"); setsearchHouseholds(fetchedHouseholdData);}} href="#">All</a></li>
                            <li><a className="dropdown-item" onClick={() => {setSearchBy("lastname"); setTypeOfInput("text");}} href="#">Last Name</a></li>
                            <li><a className="dropdown-item" onClick={() => {setSearchBy("age"); setTypeOfInput("text");}} href="#">Age</a></li>
                            <li><a className="dropdown-item" onClick={() => {setSearchBy("monthlyIncome"); setTypeOfInput("text");}} href="#">Monthly Income</a></li>
                            <li><a className="dropdown-item" onClick={() => {setSearchBy("householdSize"); setTypeOfInput("text");}} href="#">Household Size</a></li>
                        </ul>
                    </div>
                    <input type={typeOfInput} placeholder={`Enter ${searchBy}`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="form-control" />
            ...
            <div className="householdstable">
                ...
                <div className="results"> 
                    ...
                    <div className="resultsright">
                        <p>{viewHouseholds}</p>
                        <p className="resultstext" id="printtext" onClick={handlePrint}>Print Result/s</p>
                    </div>
                </div>
            </div>
                        ...
                        {filteredHouseholds.map((household) => {
                            ...
                            return (
                                <tr key={household.id}>
                                    <td>{resident ? resident.lastname : "N/A"}</td>
                                    <td>{resident ? resident.firstname : "N/A"}</td>
                                    <td>{resident ? resident.monthlyIncome : "N/A"}</td>
                                    <td>{resident ? resident.mobileNumber : "N/A"}</td>
                                </tr>
                            );
                        })}
                    </tbody>
                </table>
                <p>Generated by: Barangay Profiling System</p>
            </div>
